Model_Sim <- function(parm, N, seed = 42)
{
  set.seed(seed)
  # COVARIATES
  tau  <- tau  # trophic level for each individual i= 1,...,n
  dH_w <- dH_w # hydrogen isotope measurement of environmental water for each individual
  # PARAMETERS
  dC_M <- parm[1]
  dC_F <- parm[2]
  dC_C <- parm[3]
  ...
  # MODEL
  C_tot <- Delta_C * (tau - 2)
  dC_sim <- phi_M*(dC_M + C_tot) + phi_F*(dC_F + C_tot) + phi_C*(dC_C + C_tot)
  dH_sim <- (omega * dH_w) + (1 - omega)*(phi_M*(dH_M - dH_p) + phi_F*(dH_F - dH_p) + phi_C*(dH_C - dH_p))
  ...
  epsilon_H <- dH_obs - dH_sim
  epsilon_C <- dC_obs - dC_sim
  # LIKELIHOOD
  ...
  # MODEL RETURN
  Monitor <- c(LogPrior, LogLike, LogPosterior, dH_sim, dC_sim)
  Modelout <- list(dH_sim = dH_sim, dC_sim = dC_sim,
                   Monitor = Monitor,
                   parm = parm)
  return(Modelout)
}