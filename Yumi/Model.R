# 3 sources: marine, freshwater, coastal
Model <- function(parm, Data)
{
  ...
  src_prior  <- dC_M_prior + dC_F_prior + dC_C_prior + dH_M_prior + dH_F_prior + dH_C_prior 
  ...
  dH_p_prior    <- dnorm(dH_p, 163.7, 27, log = TRUE)
  # MODEL
#   C_tot <- Delta_C * (tau - 2)
#   u_C <- mean(phi_M*(dC_M + C_tot) + phi_F*(dC_F + C_tot) + phi_C*(dC_C + C_tot))
  ...
  LogPrior <- src_prior + phi_prior + var_prior + omega_prior + Delta_C_prior + dH_p_prior
